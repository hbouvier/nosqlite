module.exports = function () {
    ...
    Database.prototype.list = function () {
        var $this    = this,
            deferred = Q.defer();
        if ($this.sqlite) {
            sqlTables($this, function (err, result) {
                if (err) return deferred.reject(err);
    ...
    Bucket.prototype.count = function (key, options) {
        ...
        if (this.db.sqlite) {
            ...
            options.exact = isdef(options.exact) ? istrue(options.exact) : true;
            options.limit = options.limit || (options.exact ? 2 : 1000);
            sqlCount(this.db, this.name, key, options, function (err, count, elapsed) {
                if (err) return deferred.reject(err, count, elapsed);
                return deferred.resolve(count, elapsed);
            });
        } else {
            deferred.reject(new Error("Database has not been opened"), 0);
        }
        return deferred.promise;
    };
    ...
    function sqlCount(db, bucket, key, options, next) {
        var timer    = new Timer(),
            selector;
        ...
        db.sqlite.all(phrase, selector, function (err, rows) {
            ...
            if (next) {
                next(err, rows && rows.length === 1 ? rows[0].count : 0, timer.elapsed(true));
            }
        });
    }
    ...
    function sqlTables(db, next) {
        var timer    = new Timer(),
            phrase   = "SELECT name FROM sqlite_master WHERE type=$table ORDER BY name",
            selector = { $table : 'table' };

        db.sqlite.all(phrase, selector, function (err, buckets) {
            db.logger.log(err ? 'warn' : 'verbose', {
                method:'.tables',
                database: db.database,
                phrase:phrase,
            ...
            if (next) {
                next(err, buckets, timer.elapsed(true));
            }
        });
    }
